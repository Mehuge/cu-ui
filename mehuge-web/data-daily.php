...
if ($db) {
	...
	$q = mysql_query("select convert(unix_timestamp(date(timestamp))/60/60/24,UNSIGNED) AS DAY, max(current), sum(delta), sum(time_delta) AS DAY FROM cu.tracker GROUP BY DAY ORDER BY DAY");
    if ($q) {
		echo 'data:[';
        while ($r = mysql_fetch_row($q)) {
